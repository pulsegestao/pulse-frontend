import { DollarSign, Package, Bell, Sparkles } from "lucide-react";
import C from "../../theme/colors";
import MiniChart from "./MiniChart";

const kpis = [
  { label: "Vendas hoje", value: "R$ 3.840", icon: <DollarSign size={18} color={C.blue} strokeWidth={2} />, delta: "+12%", good: true },
  { label: "Itens em estoque", value: "1.247", icon: <Package size={18} color={C.graphite} strokeWidth={2} />, delta: "-3 itens", good: false },
  { label: "Alertas", value: "2", icon: <Bell size={18} color="#D97706" strokeWidth={2} />, delta: "Reposição", good: null },
];
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12, marginBottom: 20 }}>
        {kpis.map((k, i) => (
          <div key={i} style={{ background: C.gray, borderRadius: 12, padding: "14px 12px" }}>
            <div style={{ marginBottom: 6, display: "flex" }}>{k.icon}</div>
